#ifndef _Model_Pbr_bd_PS_H_
#define _Model_Pbr_bd_PS_H_

// Define In VS
#define __PS__

#ifndef USE_16_TEXUNITS
	#define USE_16_TEXUNITS 0
#endif
...
sampler2D EnvSampler			: register(s3);
	...
	#if USE_EMISSIVE == 1
		sampler2D EmissiveSampler : register(s6);
	#endif
#else
...
#if USE_16_TEXUNITS == 1
	sampler2D D2Sampler : register(s10);
	sampler2D DN2Sampler : register(s11);
#endif
...
#if IS_DETAIL == 1
	#if USE_16_TEXUNITS == 1
		float3 Calc_NormalInTangentSpace(float2 normalTexUV, float2 d0UV, float2 d1UV, float2 d2UV, float4 mask)
	#else
...
#endif
{
	...
	#if USE_16_TEXUNITS == 1
		float3 detailN2 = UnpackScaleNormal(tex2D(DN2Sampler, d2UV.xy), DN_params.b);
		detailN = (detailN2 - detailN) * mask.b + detailN;
	#endif
...
void Calc_Pbr_Tex(inout FragParams_pbr_bd params)
{
	...
	#if USE_16_TEXUNITS == 1
		float2 d2UV = params.FI.baseTC.xy * D2UV_params.xy + D2UV_params.zw;
		float4 d2Tex = tex2D(D2Sampler, d2UV);
	#endif

	params.cAlbedoTex = d0Tex;
	params.cAlbedoTex.rgb = lerp(params.cAlbedoTex.rgb, d1Tex.rgb, maskTex.g);
	#if USE_16_TEXUNITS == 1
		params.cAlbedoTex.rgb = lerp(params.cAlbedoTex.rgb, d2Tex.rgb, maskTex.b);
	#endif
...
#if IS_DETAIL == 1
		#if USE_16_TEXUNITS == 1
							Calc_NormalInTangentSpace(params.FI.baseTC.xy, d0UV, d1UV, d2UV, maskTex);
		#else
...
void Calc_Pbr(inout FragParams_pbr_bd params)
{
    ...
    float Metallic = params.cMetallicTex.r;  ///pow(params.cMetallicTex.r,2.2);
		...
		#if USE_16_TEXUNITS == 1
			float ao = params.cMetallicTex.b;
		#endif
    ...
    float3 diffuseColour[MAX_LIGHT_NUM+1];
		...
		diffuseColour[3] = GammaToLinearSpaceEasy(float3(1/255.0,1/255.0,1/255.0));
			...
			for (; l < MAX_LIGHT_NUM +1; ++l)
	    {
      	...
      	nh = nh>0.5? 2*(nh-0.5)*(nh-0.5) + 0.5 :nh;
			...
			//	specularTerm = sqrt(max(1e-4h, specularTerm));
				...
				specular[l] = specularTerm * attenColor * FresnelTerm (specColor, lh) * spec_params.rgb/255.0 * spec_params.a;
    ...
    float mip = perceptualRoughness * 6;
		...
		#if USE_16_TEXUNITS == 1
			float aoLevelMax = 10;
			params.cFinal.rgb = params.cLight.rgb * ao * (DN_params.w / aoLevelMax)  +  params.cLight.rgb * ( (aoLevelMax - DN_params.w) / aoLevelMax );
		#else
...
PS_OUT main( VS_OUT IN )
{
	...
	/// emissive
		...
		params.cFinal.rgb += params.cAlbedoTex.rgb * cEmissiveTex.r * step(imodel_userdata.w, 0.4);
	#endif
